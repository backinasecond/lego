...
public class LineFollow2 implements Behavior {
	...
	public void action() {
		...
		while(!suppressed)
		{
			...
			// If error is negative and not in the proportional range, it should steer hard left because it is far away of
			// the edge
			if (error < -PROPORTIONAL_RANGE) {
				...
				// sich nun h�chstens 90 Grad nach links drehen und schauen ob er auf dem Weg die Linie wieder findet. Wenn
				// nicht ein Drehung nach rechts um h�chstens 180 Grad. Findet er dann auch nichts, ist er am Ende der
				// Linie. --> Setze lineLeft auf true
				//Settings.PILOT.rotate(90, true);
				System.out.println("1 " + error);
				
			}
			// If error is positive and not in the proportional range, it should steer hard right because it is far away of
			// the edge
			else if (error > PROPORTIONAL_RANGE) {
				Settings.PILOT.rotate(-20, false);
				System.out.println("2 " + error);
				degreesTurnedRight += 20;
			}
			// If the error is in the proportional range the robot should steer a little right and left according to the
			// error
			else {
